...
// import { Menu } from "lucide-react" // Removed unused import
...
const Sidebar = React.forwardRef(function Sidebar(
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "group hidden text-sidebar-foreground md:flex",
        "duration-300 fixed inset-y-0 z-50 h-svh w-[--sidebar-width] transition-transform ease-in-out",
        side === "left" ? "left-0" : "right-0",
        state === "collapsed" && (side === "left" ? "-translate-x-full" : "translate-x-full"),
    ...
    >
      <div
        ...
        className={cn(
          ...
          variant === "floating" && "rounded-lg border border-sidebar-border bg-sidebar shadow"
        )}
      >
        {children}
      </div>
...
const sidebarMenuButtonVariants = cva(
  "relative flex w-full min-w-0 items-center gap-2 rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring transition-colors focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
  {
    variants: {
      variant: {
